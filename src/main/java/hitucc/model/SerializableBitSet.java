...
public class SerializableBitSet implements Cloneable, Serializable {
	 ...
	 */
	public SerializableBitSet(int capacity) {
	 ...
	 */
	public SerializableBitSet(long[] words) {
		this.words = Arrays.copyOf(words, words.length);
	}

	public static SerializableBitSet fromBinary(byte[] bytes) {
		return SerializableBitSet.fromBinary(ByteBuffer.wrap(bytes));
	}

	public static SerializableBitSet fromBinary(ByteBuffer buffer) {
		long[] words = new long[buffer.getInt()];
		for (int i = 0; i < words.length; i++)
			words[i] = buffer.getLong();
		return new SerializableBitSet(words);
	}
	...
	private void ensureCompatibility(SerializableBitSet set) {
		if (this.words.length != set.words.length)
			throw new RuntimeException(SerializableBitSet.class.getName() + " can operate only with " + SerializableBitSet.class.getName() + "s of same size.\n This words length was " + this.words.length + " while the other's word length was " + set.words.length + ".");
	}
	 ...
	 */
	public boolean intersects(SerializableBitSet set) {
	 ...
	 */
	public void and(SerializableBitSet set) {
		this.ensureCompatibility(set);
		if (this == set)
			return;
	 ...
	 */
	public void or(SerializableBitSet set) {
		this.ensureCompatibility(set);
		if (this == set)
			return;
	 ...
	 */
	public void xor(SerializableBitSet set) {
		this.ensureCompatibility(set);
		if (this == set) {
			this.clear();
			return;
	 ...
	 */
	public void andNot(SerializableBitSet set) {
		this.ensureCompatibility(set);
		if (this == set) {
			this.clear();
			return;
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof SerializableBitSet))
			return false;
		if (this == obj)
			return true;

		SerializableBitSet set = (SerializableBitSet) obj;
	...
	@Override
	public SerializableBitSet clone() {
		return new SerializableBitSet(this.words);
	}
