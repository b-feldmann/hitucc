...
public class HitUCCPeerSystem extends HitUCCSystem {
	...
	public static void start(String clusterName, int workers, String host, int port, String hostHost, int hostPort, String bindHost, int bindPort) {
		final Config config = createConfiguration(clusterName, PEER_ROLE, host, port, hostHost, hostPort, bindHost, bindPort);
		...
		Cluster.get(system).registerOnMemberUp(() -> {
			...
			system.actorOf(PeerDataBouncer.props(), PeerDataBouncer.DEFAULT_NAME + ":" + port);
		});
